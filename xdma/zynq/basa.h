...
typedef struct zynq_dev {
    ...
    //from zynq_probe, format:
    //"ZYNQ_DRV_NAMEzdev_inst-zdev_code_name-pci_domain_nr(pdev->bus)-pdev->bus->number-pdev->devfn >> 3-pdev->devfn & 0x7"
    char                zdev_name[ZYNQ_DEV_NAME_LEN];

    //from zynq_check_hw_caps, "MoonRover"
    char                zdev_code_name[ZYNQ_DEV_CODE_NAME_LEN];

    //from zynq_probe, [basa_main:: static int zynq_instance]
    int                    zdev_inst;    /* instance number */

    //from zynq_probe, zdev->zdev_version = zynq_g_reg_read(zdev, ZYNQ_G_VERSION);
    unsigned int        zdev_version;    /* version number */

    //from zynq_check_hw_caps, zdev->zdev_hw_cap = hw_cap;
    ...
    // zynq_probe() -> [basa_main::zdev_alloc()] -> zynq_zdev_init() -> ZYNQ_DMA_CHAN_NUM_MR;
    unsigned int        zdev_chan_cnt;    /* total # of channels */
    // zynq_probe() -> [basa_main::zdev_alloc()] -> zynq_zdev_init() -> hweight_long(ZYNQ_CAN_MAP_MR);
    ...
    // zdev_stats_init() for DIAG
    zynq_stats_t        stats[DEV_STATS_NUM];

    // from zynq_probe() snprintf(zdev->prefix, "%d", zdev->zdev_inst);
    char                prefix[ZYNQ_LOG_PREFIX_LEN]; // string of zdev_inst

    struct zynq_dev    	*zdev_next; // default NULL
} zynq_dev_t;
...
extern int zynq_cdev_init(void);
extern void zynq_cdev_cleanup(void);
