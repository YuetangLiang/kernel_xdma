 ...
 * remove_one() -> xdma_device_close()
 *
 * [cdev_sgdma]
 * char_sgdma_read_write() -> xdma_xfer_submit()
 *
 * [cdev_ctrl]
 * XDMA_IOCONLINE  -> xdma_device_online()
 * XDMA_IOCOFFLINE -> xdma_device_offline()
 *
 */
...
static int map_bars(struct xdma_dev *xdev, struct pci_dev *dev)
{
	...
	/* iterate through all the BARs: [0 - 5] */
	for (i = 0; i < XDMA_BAR_NUM; i++) {
		int bar_len;
